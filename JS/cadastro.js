...
function validarFormulario() {
    ...
    if (nome.value === "" || sobnome.value === "" || email.value === "" || senha.value === "" || confirma.value === "") {
        ...
        return false;
    } else if (nome.value.length < 2 || sobnome.value.length < 2 || email.value.lenght < 5) {
        alert("Os campos nome, sobrenome e email não atingiram o número mínimo de caracteres.");
        return false;
...
document.getElementById("Botaolegal").addEventListener("click", function(event) {
    ...
    if (validarFormulario()) {
        ...
        if (confirmacao) {
            ...
            console.log("Nome: " + nome.value + " " + sobnome.value);
            ...
            var listaUser = JSON.parse(localStorage.getItem("listaUser"));
            newUser = {
                nomeUsuario : nome.value + " " + sobnome.value,
